...
let get_struct_mem = function
struct_typ -> struct_typ.members
...
let semantic_check program = 
 ...
 (* above has been tested, and is working *)
  ...
  let semantic_check_func func = 
    ...
    let struct_dec = List.fold_left (fun map sd -> StringMap.add sd.name sd map)
    ...
    let get_struct_decl id =
      ...
      Struct name -> (
          try StringMap.find name struct_dec
        with Not_found -> raise (Failure ("Identifier " ^ id ^ " is undeclared"))
      )
      | _ -> raise (Failure (id ^ "is not a struct " ))
    in

    (* semantic check of expressions inside funct ions*)
